        ...
        ( sdk ) =>
        {
            ...
            document.getElementById( "image-file" )?.addEventListener( "change", ( ev ) =>
            {
                ...
                if (  inputImageFile.value ) {
                   startScan( sdk, inputImageFile.files );
                }
...
async function startScan( sdk, fileList )
{
    ...
    if ( processResult !== BlinkIDSDK.RecognizerResultState.Empty )
    {
        ...
        if ( genericIDResults.state !== BlinkIDSDK.RecognizerResultState.Empty )
        {
            ...
            const sex = genericIDResults.sex ;
            const dateOfBirth = {
             ...
             //  dateOfBirth_ =`${ dateOfBirth.year }/${ dateOfBirth.month }/${ dateOfBirth.day }`  ;
                dateOfBirth_ =` ${ dateOfBirth.day }/${ dateOfBirth.month }/${ dateOfBirth.year } `  ;



            $('#form').removeClass('d-none');
            $('#cin').val( cin_ ).removeAttr('disabled' );
            $('#nom').val( firstname_ ).removeAttr('disabled' );
            $('#prenom').val( lastname_ ).removeAttr('disabled' );
            $('#dateN').val( dateOfBirth_ ).removeAttr('disabled' );
            $('#sexe').val( sex_ ) ;
                  console.log( firstname_ +"  " + lastname_ + ' '+ dateOfBirth_ + ' '+ cin_+ ' '+ sex_);
        }
    }
    else
    {
